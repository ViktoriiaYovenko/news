...
def get_stock_rating(company, start_date: str, end_date: str):
    ...
    try:
        ...
        formatted_date = datetime.strptime(articles[0]["date"][:10], "%Y-%m-%d").strftime("%d.%m.%Y")
...
async def index(
    ...
    external_data = [{"name": name} for name in tickers]
...
async def liststock(start: str = Query(default="2025-04-01"), end: str = Query(default="2025-04-30")):
    ...
    return [
        {"name": r["name"], "date": r["date"], "rating": r["rating"], "sell": r["sell"]}
        for r in results
    ]


@app.get("/salestock", response_class=JSONResponse)
async def salestock(start: str = Query(default="2025-04-01"), end: str = Query(default="2025-04-30")):
    tickers = get_external_companies()
    results = [get_stock_rating(ticker, start, end) for ticker in tickers]
    results = [r for r in results if r and r["sell"] == 1]
    return [
        {"name": r["name"], "date": r["date"], "rating": r["rating"], "sell": r["sell"]}
        for r in results
    ]
...
@app.post("/api/zpravy", response_class=JSONResponse)
async def zpravy_post(payload: dict = Body(...)):
    recommendations = payload.get("recommendations", {})
    if not recommendations:
        return {"error": "Нет рекомендаций для обработки"}
    ...
    return {
        "received_recommendations": results,
        "status": "Обработано"
    }


@app.get("stin_backend.railway.internal/api/stocks/recommend", response_class=JSONResponse)
